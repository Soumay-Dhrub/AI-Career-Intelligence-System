...
def _load_xgboost() -> type | None:
    try:
        ...
        if "libomp.dylib" in error_text or "OpenMP" in error_text:
            print(
                "  [burnout] xgboost is installed but missing libomp. "
                "Install it with `brew install libomp` or skip XGBoost."
            )
        else:
            print(f"  [burnout] xgboost not available — skipping XGBoost ({exc.__class__.__name__})")
        return None

XGBClassifier = _load_xgboost()
_HAS_XGB = XGBClassifier is not None
...
def train(df: pd.DataFrame) -> None:
    ...
    for name, clf in candidates.items():
        ...
        print(f"  [burnout] {name}: Accuracy={accuracy:.4f}, Precision={precision:.4f}, Recall={recall:.4f}, F1={f1:.4f}")
        print(classification_report(y_test, preds, target_names=["Low", "Medium", "High"], zero_division=0))

        if f1 > best_f1:
            best_f1, best_name, best_model = f1, name, clf

    print(f"  [burnout] Best model: {best_name} (F1={best_f1:.4f})")
